...
export default class WordStore {
	...
	public editWord(newWord: Word) {
		if (this.words.has(newWord.id)) {
			...
			this.words.set(newWord.id, { id: newWord.id, word: newWord.word, definition: newWord.definition, sentences: newWord.sentences });
		}
	}
	...
	public addWord(id: string, word: string, definition: string, sentences: string[] = []): boolean {
	...
	private saveWords() {
		...
		this.saveTimeout = setTimeout(() =>
			localStorage.setItem(STORAGE_KEY, JSON.stringify(Array.from(this.words.entries())))
			, 500
		);
	}
}
