...
export default function Page() {
	...
	const handleAddWord = () => {
		...
		const maxid = words.length ? Math.max(...words.map(w => Number(w.id))) : 0;
		const nextid = maxid + 1;
		wordsStore.addWord(String(nextid), trimmedWord, trimmedDefinition);
		setWords([...words, { id: String(nextid), word: trimmedWord, definition: trimmedDefinition }]);
		setNewWord("");
		setNewDefinition("");
	};

	const handleEditClick = (selectedWord: Word) => {
		if (editing === true && editingWord !== null) { // editing was true, then clicked = saved
			wordsStore.editWord(editingWord);
			setWords(words.map(word => word.id === editingWord.id ? editingWord : word));
		} else {
	...
	return (
		<div className="min-h-screen bg-zinc-950">
			<div className="container mx-auto px-4 py-8 text-white">
				...
				{/* Add Word Drawer */}
				<AddWordButton newWord={newWord} newDefinition={newDefinition} setNewDefinition={setNewDefinition} setNewWord={setNewWord} handleAddWord={handleAddWord} />
					...
					{words && words.length > 0 && words.map((word) => (
						<Drawer key={word.id}>
							<DrawerTrigger asChild>
								<button
									className="py-2 px-4 border border-zinc-800 rounded-lg hover:bg-zinc-800/90 transition-colors text-center bg-zinc-900 w-fit"
								>
									{word.word}
								</button>
							</DrawerTrigger>
								...
								<div className="mx-auto w-full max-w-sm bg-zinc-950">
									...
									<DrawerFooter>
										<div className="flex items-center gap-1 w-full">
											<Button className="w-full" onClick={() => handleEditClick(word)} variant={editing ? "default" : "secondary"}>{editing ? "Save" : "Edit"}</Button>
											<Button className="w-full" onClick={() => handleDeleteWord(word.id)} variant="destructive">Delete</Button>
										</div>
										<DrawerClose asChild>
